	...
	public sealed class ClassTypeReferencePropertyDrawer : PropertyDrawer {
		...
		public static string DrawTypeSelectionControl(Rect position, GUIContent label, string classRef, ClassTypeConstraintAttribute filter) {
			...
			if (triggerDropDown) {
				...
				var filteredTypes = ClassTypeReference.GetFilteredTypes(filter).ToList();
		...
		private static void DrawTypeSelectionControl(Rect position, SerializedProperty property, GUIContent label, ClassTypeConstraintAttribute filter) {
			try {
				...
				ClassTypeReference.ExcludedTypeCollectionGetter = null;
			}
		}
		...
		public static string FormatGroupedTypeName(Type type, ClassGrouping grouping) {
			...
			switch (grouping) {
				...
				case ClassGrouping.ByAddress:
					var attClassTypeAddress = type.GetCustomAttributes(typeof(ClassTypeAddress), false);
					if(attClassTypeAddress.Length > 0)
                    {
						return ((ClassTypeAddress)attClassTypeAddress[0]).Address;
					}
